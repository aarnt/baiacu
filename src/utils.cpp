 ...
 * If it fails to connect to the internet, uses the available "./.config/baiacu/distro_rss.xml"
 * The result is a QString containing the RSS News Feed XML code
 */
QString utils::retrieveDistroNews(bool searchForLatestNews)
{
  const QString ctn_OPENBSD_RSS_URL = "https://undeadly.org/cgi?action=rss";
  QString res;
  QString tmpRssPath = QDir::homePath() + QDir::separator() + ".config/baiacu/.tmp_distro_rss.xml";
  QString rssPath = QDir::homePath() + QDir::separator() + ".config/baiacu/distro_rss.xml";
  QString contentsRss;
  ...
  else
  {
    ...
    //Maybe we have a file in "./.config/baiacu/distro_rss.xml"
    if (fileRss.exists())
    {
      res = contentsRss;
    }
...
{
  QString html;
  html = "<p align=\"center\"><h2>" + StrConstants::getOpenBSDNews() + "</h2></p><ul>";
