...
QSet<QString>* Package::getUnrequiredPackageList()
{
  ...
  {
    //We need to get only the pkg names
    //pcmanfm-qt-1.0.0    file manager and desktop icon manager
    sl = packageTuple.split(" ", Qt::SkipEmptyParts, Qt::CaseInsensitive);
...
/*
0123456789
zimg-3.0.4   scaling, colorspace conversion, and dithering library
*/
  ...
  if(!pkgList.isEmpty())
  {
    ...
    {
      pkgComment = "";
      space = packageTuple.indexOf(" ");
      nameVersion = packageTuple.left(space);
      pkgDescription = "";
      ...
      pkgVersion = nameVersion.mid(lastSep+1, nameVersion.size()-lastSep+1);
      pkgOrigin = "";
      pkgStatus = ectn_INSTALLED;
      pkgDownloadedSize = 0;
      pkgInstalledSize = 0;

      pkgInstallDate = getPackageInstallDate(nameVersion);
...
/*
 * Finds the OpenBSD pkg install date looking into /var/db/pkg/ directory
 */
QString Package::getPackageInstallDate(const QString &pkgNameVersion)
{
  QFileInfo fi("/var/db/pkg/" + pkgNameVersion + "/+CONTENTS");
...
QList<PackageListData> * Package::parsePackageTuple(const QStringList &packageTuples, QStringList &packageCache)
{
  ...
  for(QString packageTuple: packageTuples)
  {
    ...
    if (packageTuple.contains("(installed)"))
    {
      pld.status = ectn_INSTALLED;
      packageTuple.remove("(installed)");
    }
    else
    {
    ...
    if (pld.status == ectn_INSTALLED)
    {
      pld.installedOn = getPackageInstallDate(pkgName + "-" + pkgVersion);
    }
    ...
    //std::cout << pld.name.toLatin1().data() << "<->" << pld.version.toLatin1().data() << std::endl;
    res->append(pld);
  }
...
{
  QString aux = extractFieldFromInfo("Size", pkgInfo);
  bool isMega = (aux.indexOf("MiB", Qt::CaseInsensitive) != -1);
  aux = aux.section(QRegularExpression("\\s"), 0, 0);
  bool ok;
  double res = aux.toDouble(&ok);
...
QString Package::getFiles(const QString &pkgInfo)
{
  return extractFieldFromInfo("Files", pkgInfo);
}
...
/*
 * Iterates over the WeirdVersionPkgList to see if given pkg is weird
 */
bool Package::isWeirdVersionPkg(const QString& packageTuple)
{
  for (QString p: getWeirdVersionPkgList())
  {
    if (packageTuple.contains(p)) return true;
  }
...
/*
 * Returns the names of packages that have uncommon version names
 */
QStringList Package::getWeirdVersionPkgList()
{
