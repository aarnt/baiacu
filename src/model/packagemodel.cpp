...
const QIcon& PackageModel::getIconFor(const PackageRepository::PackageData& package) const
{
  switch (package.status)
  {
    ...
    case ectn_INSTALLED:
      ...
      if (package.required)
      {
        if (!package.installedOn.isEmpty())
          return m_iconInstalled;
        else
        {
          //PackageRepository::PackageData *pPackage = const_cast<PackageRepository::PackageData *>(&package);
          //pPackage->status = ectn_NON_INSTALLED;
